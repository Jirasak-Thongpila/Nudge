...
import '../widgets/nudge_drawer.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget _buildNotificationCenterTab() {
                      ...
                      child: FilledButton.icon(
                        ...
                        onPressed: () {
                          ...
                          if (taskId != null) {
                            ...
                            if (found != null) _onStartFocusSession(found);
                          }
                        },
                      ),
                    ),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              FilledButton(onPressed: _fetchDashboard, child: const Text('ลองใหม่')),
            ],
          ),
        ),
      );
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                _buildTaskListSection('งานถัดไป (Next)', _dashboardData!.next, icon: Icons.arrow_forward_rounded),
                const SizedBox(height: 16),
                _buildTaskListSection('งานที่ยังไม่เร่งด่วน (Later)', _dashboardData!.later, icon: Icons.access_time_rounded),
  ...
  Widget _buildCurrentTab() {
    switch (_selectedNavIndex) {
      ...
      case 2:
        return _buildNotificationCenterTab();
      case 0:
      default:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: NudgeDrawer(
        ...
        onSelectTab: (i) => setState(() => _selectedNavIndex = i),
        onAddTask: () async {
          final created = await Navigator.push<bool>(
            context,
            MaterialPageRoute(builder: (_) => AddTaskScreen(apiClient: widget.apiClient)),
          );
          if (created == true) _fetchDashboard();
        },
        onRefresh: _fetchDashboard,
      ...
      // ── Body (lazy tab creation — only builds when first visited) ──────────
      body: _buildCurrentTab(),
      // ── FAB (Dashboard tab only) ───────────────────────────────────────────
